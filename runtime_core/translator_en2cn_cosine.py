...
# Cosine Annealing with Warm-up 版本
...
def get_config(debug=True):
    if debug:
        ...
        save_dir = f"save/models-cosine/{experiment_name}/"
        return{
            'peak_lr': 3e-4,           # 峰值学习率（可调）
...
# ============================================================
# Cosine Annealing with Warm-up 调度器
# ============================================================
optimizer = torch.optim.Adam(model.parameters(), lr=config['peak_lr'] if not DEBUG else config['lr'], eps=1e-9)
...
if not DEBUG:
    ...
    print(f"Cosine Warmup scheduler initialized: peak_lr={config['peak_lr']}, warmup_steps={warmup_steps}, total_steps={total_steps}")
else:
    scheduler = None
...
def save_experiment_config(config, final_val_bleu=None, final_test_bleu=None):
    config_path = f"save/models-cosine/{config['experiment_name']}/config.json"
...
# Training model
print(f">>>>>>> start train (experiment: {config.get('experiment_name', 'default')})")
...
for epoch in range(initial_epoch, config['num_epochs']):
    ...
    for batch in batch_iterator:
        ...
        # Update learning rate scheduler (cosine warmup: linear up, cosine down)
...
# Save experiment config for version tracking
